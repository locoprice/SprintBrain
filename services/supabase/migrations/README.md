...
| `20260613000000_hotfix_readd_is_shared_premature_b6.sql` | ✅ applied 2026-06-13 | **Hotfix (v2.61.1):** re-adds `snippets.is_shared` (`boolean NOT NULL DEFAULT false`). B6 dropped the column before stale clients stopped requesting it — the deployed dashboard (< v2.61.0) selects `…,is_shared,…` and the installed extension (< v2.60.0) filters `or=(user_id.eq.<uid>,is_shared.eq.true)`, so post-drop both 400'd (`column snippets.is_shared does not exist`) → "Failed to load snippets" / empty popup. Confirmed via API+Postgres logs. The column is **access-control-inert** — B5's `is_shared = true` RLS read branch is **not** re-added, so folder ACL remains the only org-share path (no cross-tenant exposure). Verified: previously-failing dashboard + extension query shapes now return rows. **Re-drop (re-apply B6) only after client adoption is confirmed** per `docs/PHASE_B_SOAK.md` C4/C8. |

> **Phase B (B0–B6) shipped, then B6's column drop was reversed by the 2026-06-13 hotfix above** — stale dashboard/extension builds were still reading `is_shared`, so the column is temporarily restored for read-compatibility. Sharing remains folder-ACL only (B5's RLS read-branch retirement stands). Re-drop once adoption is confirmed. History of the soak, exit criteria, and the execution log live in `docs/PHASE_B_SOAK.md`.
>
...
| `20260923000000_memory_mcp_write.sql` | ✅ applied 2026-09-23 | Recorded as `memory_mcp_write` (`20260923115225`), applied by a parallel session rather than the one that wrote it. The MCP write surface (MEMORY-002 S8). Moves the body of `memory_save_shard` into `app.memory_write_shard(p_uid, …)` and leaves `memory_save_shard` as a wrapper that resolves `auth.uid()`, so session callers are untouched and there is still exactly one function that writes row + version + audit. Adds `public.memory_mcp_save()`, which requires a token carrying the `write` scope and only ever creates. Re-creates `memory_issue_token` with a third `p_scopes` parameter defaulting to `{read}` (dropped first: a defaulted third argument would make every existing two-argument call ambiguous). **Verify `memory_save_shard` still saves from the dashboard immediately after applying** — it is the one function this touches that was already in use. |
