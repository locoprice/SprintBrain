...
(function (root) {
   ...
   */
  function tally(owners, currentUserId) {
    var me = String(currentUserId || '');
    var out = { total: owners.length, personal: 0, shared: 0, owners: 0 };
    var seen = {};
  ...
  function stats(rows, currentUserId) {
    ...
    var owners = [];
    for (var i = 0; i < groups.length; i++) owners.push(groups[i].owner);
    return tally(owners, currentUserId);
  }

  /**
   * The same breakdown for a library whose rows do not fold: a prompt carries
   * no translations, so one row is one prompt. Grouping them by trigger base
   * would merge two prompts whose shortcuts happen to share a stem, and print
   * a total that disagrees with the count on the Prompts tab.
   */
  function rowStats(rows, currentUserId) {
    var list = rows || [];
    var owners = [];
    for (var i = 0; i < list.length; i++) owners.push(ownerOf(list[i]));
    return tally(owners, currentUserId);
  }

  /** The one breakdown string every library prints, so they cannot drift. */
