...
// Shared by the Chrome extension (content script via the manifest, popup via
// popup.html) and the dashboard's live preview, which loads it from /extension/.
//
// Supported syntax:
...
(function(root) {
  ...
  // The React dashboard (FormNumberDialog) builds number fields and cannot
  // import extension source, so src/lib/formNumberToken.ts mirrors this writer
  // and formNumberField.test.ts pins the two against each other. Same
  // arrangement as buildFormMenuToken and buildFormButtonToken: one writer
  // here, so the two cannot drift into emitting different tokens for the same
  // choices.
  //
