...
- Google sign-in (v3.33.0): `components/auth/GoogleSignIn.tsx` on `/login` and `/signup` calls `signInWithOAuth` and returns through `/auth/callback?provider=google`. Failures reach `/login` as a short `?error=` code (`lib/authCallback.ts`), never as free text. GoTrue rewrites `full_name` and `avatar_url` at every Google sign-in, so Settings also writes `chosen_full_name` / `chosen_avatar_url` and the `app.keep_chosen_profile` trigger on `auth.users` puts the person's choice back (migration `20260919120000_google_sign_in`). Readers on every surface keep reading `full_name` and `avatar_url`.
- Password choice (v3.34.0): `/login` and `/signup` open on two tabs, Email link and Password (`components/auth/AuthMethodTabs.tsx`, `PasswordInput.tsx`). Password sign-up (`signUp`) confirms the address by email. Settings > Security carries the Password card (`features/settings/PasswordPanel.tsx`, `updateUser({ password })`), the only place an existing account gets a password; "Forgot password?" emails a sign-in link that lands there (`/settings?tab=security`). Wording rules live in `lib/passwordAuth.ts`.
