...
/**
 * How a memory item arrived. Retrieval treats all four identically; the
 * distinction is for the reader and for filters. Mirrors the
 * `memory_shards_kind_known` CHECK, so widening this means widening that too.
 */
export type MemoryItemKind = 'fact' | 'note' | 'document' | 'conversation';

/**
 * A container for one body of knowledge (`public.memory_spaces`).
 *
 * Personal. Sharing is a separate, explicit act that lands with
 * `memory_space_shares`; nothing here grants a teammate anything.
 */
export interface MemorySpace {
  ...
  name: string;
  description: string;
  /** Keyword key resolved per surface, never markup. See features/memory/spaceIcon.tsx. */
  ico: string;
  /** Exactly one live default per user, created on the first item write. */
  is_default: boolean;
  created_at: IsoDateTime;
  updated_at: IsoDateTime;
  /** Trash marker. Rows stay readable to their owner so the trash view can list them. */
  deleted_at: IsoDateTime | null;
}

/**
 * One memory item: a CRUD row of `public.memory_shards`.
 *
 * Distinct from `MemoryShard` in `lib/memory/engine.ts`, which is the same row
 * projected for ranking (id, name, summary, body, tokens, labels) and carries
...
export interface MemoryItem {
  ...
  body: string;
  kind: MemoryItemKind;
  /** Free-form and caller-owned, capped at 4 KB by a CHECK constraint. */
  metadata: Record<string, unknown>;
  /** ceil(chars / 4), generated by the database. Never computed client-side. */
  token_estimate: number;
  /** Always attached, never evicted. */
  pinned: boolean;
...
/** Per-space totals for the index cards and the detail tiles. */
export interface MemorySpaceTotals {
  items: number;
  tokens: number;
}
