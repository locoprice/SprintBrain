...
// tested before the index, or every space reads "Search spaces…".
describe('searchPlaceholder', () => {
  it('names the section you are in', () => {
    ...
    expect(searchPlaceholder('/memory')).toBe('Search spaces…');
  });

  it('switches to the items inside a space', () => {
    expect(searchPlaceholder('/memory/abc-123')).toBe('Search this space…');
  });
